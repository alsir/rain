    ...
    <div class="content-header row">
        <div class="content-header-left col-md-9 col-12 mb-2">
            <div class="row breadcrumbs-top">
                <div class="col-12">
                    <h2 class="content-header-title float-left mb-0">كابون </h2>
                </div>
            </div>
        </div>
    </div>
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ $coupon->amount }}</td>
                                                <td><button class="btn btn-success" name="edit_button"
                                                    value="{{ $coupon->id }}" data-toggle="modal"
                                                    data-target="#edit_modal"><i class="fa fa-edit"></i></button>
                                                <button class="btn btn-danger mr-2"
                                                    onclick="if(confirm('هل أنت متأكد ؟')){document.getElementById('delete-users_{{ $coupon->id }}').submit();}else{
                                            event.preventDefault();}"><i
                                                        class="fa fa-trash"></i></button>
                                                <form id="delete-users_{{ $coupon->id }}"
                                                    action="/admin/coupon/{{ $coupon->id }}" method="POST"
                                                    class="d-none">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
        ...
        <div class="modal-dialog modal-dialog-scrollable modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <button coupon="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form class="form form-vertical" action="/admin/coupon" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="form-body">
                            <div class="row">
                                <div class="col-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text"  class="form-control @error('id') is-invalid @enderror"
                                            name="id" placeholder="id" value="{{ old('id') }}" required>
                                    </div>
                                </div>
                                <div class="col-6">
                                    <div class="form-group">
                                        <label for="first-name-vertical">القيمة</label>
                                        <input type="number" step="0.01" min="0" max="10" class="form-control @error('amount') is-invalid @enderror"
                                            name="amount" placeholder="لقيمة" value="{{ old('amount') }}" required>
                                    </div>
                                </div>
                                <div class="col-12">
    ...
    <script>
        $(document).ready(function() {
            $("button[name='edit_button']").click(function() {
                ...
                $.get("/admin/coupon/" + edit_val + "/edit", function(data, status) {
